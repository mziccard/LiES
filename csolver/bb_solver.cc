...
using namespace std;
  ...
  bb_solver_t::bb_solver_t(csp_t mcsp): backtrack_solver_t(mcsp), min_value(LONG_MAX) {
    min_solutions = (int*)malloc(csp.vnum * sizeof(int));
    memset(min_solutions, 0x0, csp.vnum * sizeof(int));
  }

  long bb_solver_t::evaluate_node(int* solutions, int last) {
    long value = 0;
    unsigned int i;
    for (i = 0; i <= last; i++) {
  ...
  bool bb_solver_t::solve(int* solutions) {
    stats.start_timer();
